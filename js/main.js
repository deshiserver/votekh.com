;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-voteKh-nav-toggle', function(event){
	...
	var clickMenu = function() {
		...
		$('a:not([class="external"])').click(function(event){
		    ...
		    if ( navbar.is(':visible')) {
		    	...
		    	$('.js-voteKh-nav-toggle').removeClass('active');
		    }
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
		   	...
		   	var header = $('#voteKh-header'),
				scrlTop = $(this).scrollTop();

			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top voteKh-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top voteKh-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top voteKh-animated slideInDown slideOutUp');
					}, 100 );
				}
			} 
	...
	var servicesAnimate = function() {
		...
		if ( $('#voteKh-our-services').length > 0 ) {	
			$('#voteKh-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#voteKh-our-services').length > 0 ) {
			$('#voteKh-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#voteKh-features').length > 0 ) {	
			$('#voteKh-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#voteKh-features').length > 0 ) {
			$('#voteKh-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#voteKh-testimonials').length > 0 ) {	
			$('#voteKh-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#voteKh-testimonials').length > 0 ) {
			$('#voteKh-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#voteKh-pricing').length > 0 ) {	
			$('#voteKh-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#voteKh-pricing').length > 0 ) {
			$('#voteKh-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#voteKh-press').length > 0 ) {	
			$('#voteKh-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#voteKh-press').length > 0 ) {
			$('#voteKh-press').waypoint( function( direction ) {
